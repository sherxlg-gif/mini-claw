<p align="center">
  <a href="https://happyclaw.cc/">
    <img src="web/public/icons/logo-1024.png" alt="Miniclaw Logo" width="128" />
  </a>
</p>
...
<p align="center">
  <strong>自托管、多用户、Pi Agent 工作台</strong>
  <br />
  让 Pi Agent 通过 Web 与 7 种消息渠道长期在线，在宿主机或 Docker 沙箱中安全执行任务。
</p>
...
Miniclaw 是一个以 Pi Agent Runtime 为核心的自托管 AI 智能体系统。它把长期运行的 Agent、工作区、能力、记忆和自动化任务封装成多用户服务，让你可以从浏览器、飞书、Telegram、QQ、钉钉、微信、Discord 或 WhatsApp 使用同一套工作台。

Miniclaw 不是一个简单的聊天 API Wrapper。智能体运行在 Pi Agent Runtime 中，可以读写项目文件、执行终端命令、调用兼容能力、加载 Skills，并在多个独立工作区和会话之间保持清晰的权限与上下文边界。

### 为什么使用 Miniclaw

- **Pi 原生运行时** — 使用版本锁定的 Pi Coding Agent、ModelRuntime、SessionManager 与 Extension/Skill 系统；Anthropic 仍可作为模型提供商。
...
| 模块           | 主要能力                                                                                |
| -------------- | --------------------------------------------------------------------------------------- |
| **智能体**     | 主 Miniclaw 对话式创建/编辑、自定义智能体、头像、结构化提示词、AI 优化、版本历史与恢复 |
| **工作区**     | 智能体归属、宿主机/容器执行、独立目录、项目环境变量、项目上下文、多会话               |
| **能力治理**   | Pi Skills、系统/用户能力工具、兼容插件数据与最终生效预览；智能体工具权限完整开放       |
...
Miniclaw 使用统一的 `智能体 → Workspace → Runtime Session` 层级：
...
- **内置 Miniclaw** 是不可删除的平台主 Agent，拥有代码级固定身份，理解
  Workspace、Session、Agent Builder、渠道、任务、Skills/MCP、Provider 和权限
  模型；这些规则不存放在可遗忘的 Memory 中。
- **首次唤醒** 只面向 Home Workspace 中经过宿主验证的实际 owner。Miniclaw
  用简短的“刚醒来”话语询问称呼，并通过专用 Owner Profile 保存；并发会话使用
  onboarding lease，后续新 Session 或服务重启不会重复表演。清空称呼也不会重置
  onboarding。
- **智能体** 保存长期身份和能力策略。主 Miniclaw 智能体始终存在，也可以创建代码审查、研究、运维等自定义智能体。
- **对话式智能体构建器** 在主 Miniclaw 名下的所有 Workspace 主会话与普通 Runtime Session 中生效，允许用户多轮讨论需求、保存并预览智能体草稿，并在后续人类消息逐字回复一次性确认口令后创建或更新智能体；定时任务与 Sub-Agent 不能代替用户发布。
- **Home Workspace** 是不可删除的系统工作区，并永久归属内置 Miniclaw。
- **自定义 Agent** 创建后没有隐式 Workspace、Session 或 Memory；用户需要显式
  新建 Workspace 或迁移一个非 Home Workspace，避免继承 Miniclaw 上下文。
...
- 托管项目上下文或管理员宿主机兼容上下文。
- 用户 Skills、Miniclaw 能力工具与兼容插件数据；智能体始终拥有完整工具权限。

配置页可以针对具体工作区预览最终生效能力，包括项目指令文件、Skills、项目 MCP、用户能力和同名覆盖来源。旧 `.claude` 配置仍作为兼容输入读取。

### Skills、MCP 与 Plugins

Miniclaw 区分不同层级的能力来源：

| 来源                    | 说明                                                              |
| ----------------------- | ----------------------------------------------------------------- |
| **内置能力**            | 随 Miniclaw 或智能体容器提供的基础工具与 Skills                    |
| **用户 Skills**         | 从技能市场、HTTPS Git 仓库或 ZIP 导入，按用户隔离并记录来源与版本 |
| **项目上下文**          | 工作区目录里的指令文件、Skills 和 MCP 配置；旧 Claude 路径兼容读取 |
| **用户 MCP**            | 当前用户维护的 MCP Server，只进入允许使用它的智能体               |
| **系统 MCP**            | 管理员维护，默认仅管理员可用，显式共享后普通成员才能使用          |
| **兼容插件数据**        | 系统 catalog 与用户启用状态分离，通过版本化运行快照加载             |
...
Miniclaw 可以同时配置多个 Provider；Pi ModelRuntime 负责模型解析、端点和凭据注入，并按策略为新会话选择健康的 Provider。

| 类型               | 配置方式                                                               |
| ------------------ | ---------------------------------------------------------------------- |
| **Anthropic 官方** | Claude 模型、OAuth、一键登录、`setup-token` / `.credentials.json` 或 API Key |
| **第三方兼容端点** | 填写 API Endpoint、Auth Token 和模型名称，可启用 1M 上下文             |

第三方 Provider 会自动预填兼容端点环境变量，包括主模型映射、上下文窗口、自动压缩、超时和非必要流量设置。高级设置会展示所有预填值，用户可以编辑、恢复默认值或增加自定义 Header 等环境变量。
...
- **执行类型** — 智能体任务运行完整 Pi Agent；Script 任务直接执行 Shell。
- **上下文模式** — `group` 注入工作区主会话；`isolated` 每次创建独立队列、Pi session 和 IPC 命名空间。
...
> Pi Agent Runtime、Pi subagents 扩展和 agent-browser 已锁定在 Agent Runner 依赖中，不需要另外安装 Claude Code CLI。
...
1. 创建第一个管理员账户。
2. 配置 Anthropic 官方账号或第三方兼容 Provider。
...
manifest。镜像构建时会解析 Pi Coding Agent、Pi subagents、agent-browser、
...
Miniclaw 优先通过 Web 设置管理配置，不要求用户维护一组庞大的环境变量。
...
| **设置 → 主 Miniclaw** | 默认智能体的 Skills 与 MCP                             |
| **设置 → 执行与容量**   | 超时、并发、上下文窗口和运行限制                      |
| **设置 → 宿主机集成**   | 管理员纯宿主机模式、Host 目录与项目上下文来源         |
...
├── sessions/          # 主会话与 Runtime Session 的 Pi 数据
...
```mermaid
flowchart LR
    Web["Web / PWA"] --> Core["Miniclaw Node Service"]
    IM["7 个 IM 渠道"] --> Core
    ...
    Host --> Pi["Pi Agent Runtime"]
    Container --> Pi
    Pi <--> MCP["Capability Tools / IPC"]
    MCP --> Core
...
主服务始终使用 Node.js 运行，负责认证、路由、队列、调度、渠道连接、Provider 池、用量和持久化。Docker 只用于隔离智能体执行环境，不承载 Miniclaw 主服务。
...
| **智能体运行时** | Pi Coding Agent、Pi ModelRuntime、Pi Skills/Extensions、Capability Tools、文件 IPC |
...
Miniclaw 会执行代码和访问第三方消息平台，部署前请理解以下边界：
...
| [Plugin 自动化设计记录](docs/claude-code-plugin-automation-design.md) | 兼容 Plugin catalog 与运行快照的历史设计          |
...
<details>
<summary><strong>需要另外安装 Pi 或 Claude Code CLI 吗？</strong></summary>

不需要。官方 Agent 镜像会在每次发布时安装锁定版本的 Pi Coding Agent、Pi
subagents 和 agent-browser，并把实际版本写入
...
<details>
<summary><strong>可以使用第三方兼容接口吗？</strong></summary>

可以。在“设置 → 模型与提供商”选择第三方，只需填写 Endpoint、Auth Token 和模型名；兼容端点环境变量由系统预填，也允许在高级设置中编辑。
...
- [Pi Coding Agent](https://github.com/badlogic/pi-mono) 与 [Pi subagents](https://github.com/tintinweb/pi-subagents) 提供 Agent Runtime 和子代理扩展。
- 感谢所有渠道 SDK、开源依赖、Issue 提交者和贡献者。

Miniclaw 是独立开源项目，与 Pi、Anthropic 及各消息平台不存在官方隶属、授权或背书关系。相关名称和商标归各自权利人所有。
